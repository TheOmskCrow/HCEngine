...
namespace hc3d {
	...
	class HC3D_API Point3D : public Vector3D
	{
	public:
	...
	inline HC3D_API Vector3D operator *(float t, const Vector3D& v)
	{
		return (Vector3D(t * v.x, t * v.y, t * v.z));
	}

	inline HC3D_API Point3D operator *(float t, const Point3D& p)
	{
		return (Point3D(t * p.x, t * p.y, t * p.z));
	}

	inline HC3D_API float Dot(const Vector3D& v1, const Vector3D& v2)
	{
		return (v1 * v2);
	}

	inline HC3D_API Vector3D Cross(const Vector3D& v1, const Vector3D& v2)
	{
		return (v1 % v2);
	}

	inline HC3D_API float Magnitude(const Vector3D& v)
	{
		return (sqrtf(v.x * v.x + v.y * v.y + v.z * v.z));
	}

	inline HC3D_API float InverseMag(const Vector3D& v)
	{
		return (1.0F / sqrtf(v.x * v.x + v.y * v.y + v.z * v.z));
	}

	inline HC3D_API float SquaredMag(const Vector3D& v)
	{
