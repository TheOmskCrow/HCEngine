...
bool Material::SetDiffuse(GLuint diffuseMap, Vector3D farColor, float farDist) {
	if (MatLib::IsUsed(diffuseMap) || _initialized) return false;
	_initialized = true;
	this->diffuseMap = diffuseMap;
	this->diffuseFarColor = farColor;
	this->diffuseFarDist = farDist;
	return MatLib::Use(diffuseMap);
}

bool Material::SetDiffuse(std::string diffuseMapName, Vector3D farColor, float farDist) {
	if (_initialized) return false;
	_initialized = true;
	try {
		this->diffuseMap = LoadTexture(diffuseMapName.c_str());
		this->diffuseFarColor = farColor;
		this->diffuseFarDist = farDist;
